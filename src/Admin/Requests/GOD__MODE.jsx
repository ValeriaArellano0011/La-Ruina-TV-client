...
export const GOD__MODE = () => {
  ...
  function handlerClick(){
    const free = JSON.stringify({userId: user.userId, userAlias: user.userAlias, email:user.email, role:{ role: "admin",userMode:"free"}})
    const subscriber = JSON.stringify({userId: user.userId, userAlias: user.userAlias, email:user.email, role:{ role: "admin",userMode:"subscriber"}})
    const admin = JSON.stringify({userId: user.userId, userAlias: user.userAlias, email:user.email, role:{ role: "admin",userMode:"admin"}})
